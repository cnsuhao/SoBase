...
SoPackageFile::OperationResult SoPackageFile::Read(void* pBuff, soint64 nElementSize, soint64 nElementCount, soint64& nActuallyReadCount, stReadSingleFile& theFile)
{
	...
	if (theFile.pFileBuff == 0)
	{
		...
		int nResult = uncompress((Bytef*)m_pTempBuff_SrcFile, &nSizeAfterUncompress, (Bytef*)m_pTempBuff_AfterCompress, (uLongf)theFileInfo.nEmbededFileSize);
		if (nResult != Z_OK)
		{
			LeaveCriticalSection(&m_Lock);
		...
		theFile.pFileBuff = (char*)malloc((size_t)theFileInfo.nOriginalFileSize);
		memcpy(theFile.pFileBuff, m_pTempBuff_SrcFile, (size_t)theFileInfo.nOriginalFileSize);
		LeaveCriticalSection(&m_Lock);
	}
	...
	memcpy(pBuff, theFile.pFileBuff+theFile.nFilePointer, (size_t)nActuallyReadSize);
	//读取完毕。
...
void SoPackageFile::TryResizeTempBuff_SrcFile(soint64 nDestSize)
{
	...
	if (nRealDestSize > 0)
	{
		...
		m_pTempBuff_SrcFile = (char*)malloc((size_t)nRealDestSize);
...
void SoPackageFile::TryResizeTempBuff_AfterCompress(soint64 nDestSize)
{
	...
	if (nRealDestSize > 0)
	{
		...
		m_pTempBuff_AfterCompress = (char*)malloc((size_t)nRealDestSize);
